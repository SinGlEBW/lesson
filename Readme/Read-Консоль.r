...
git log --graph посмотреть кто на каких узлах (комитах). Выйти на q нажать
...
git checkout <далее название ветки на которую ходим перейти>. Если перед именем указать -b то мы сразу создаём и переходим(branch можно не использовать)
Может случиться такая ситуация когда мы зайдём в прошлый комит где код был на стадии начальной разработки. При этом сделаем некоторые изменения.
Если попытаться перейти на коммит финальной части кода, то будет ошибка. Потребуеться вернуть изменёный комит в прежнее состояние. Что бы откатить
git checkout <путь> - откатит старый комит, если были изменения
...
После объединения master приобретает название (master | MERGING). Как только будет всё закомичего статент снова master. Странно что 
добавленый код добавляет опознавательные коментарии начала и конца кода
