...
/*------------------------------------------------------------------------- */
    ...
    function animation() {
      ...
      for (let i = 1; i < dots.length; i++) {
         /*1 й цикл перебирает переменные, 2й цикл перебирает те же переменные но со сдвигом
            так можно сравнивать каждый элемент массива друг с другом. 1й элемент 1го цикла, со всеми элементами
            2го цикла, потом 2й элемент 1го цикла со всеми элементами 2го цикла и т.д. 
            В циклах встречается момент когда значение сравнивается сама с собой, прежде чем идти по циклу дальше.
            Если это сравнение не требуется то и используется пропускание цикла через continue
         */
         let acc = {x: 0, y: 0}// отвечает за ускорение частиц
  
        for (let j = 0; j < dots.length; j++) {
          if (i == j) continue;
          let [a, b] = [dots[i], dots[j]]; //достаём объекты с координатами. 1й об. в А и перечисление в В. 2й об. в А и перечисление в B и т.д.
 
          let delta  = {x: b.pos.x - a.pos.x, y: b.pos.y - a.pos.y} // вычисляет разницу между позицией 1й элемента и всех последующих и т.д
          let dist   = Math.sqrt( delta.x ** 2 + delta.y ** 2) || 1;//Пифагор. Гипотенуза служит расстоянием
