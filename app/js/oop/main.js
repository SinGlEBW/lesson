...
Теперь о классах......
    ...
    В классах для этого используется пространство за пределами constructor. Правда используя синтаксис ES6
...
/*  
  1.Всё что указываеться в методе constructor отображаеться на нулевом уровне как у родительского класса, так и у
    наследованного. Исключение создание безымянной функции за пределами constructor или свойства без this и let,
    если создать за пределами то будет тот же эффект что и создание в конструкторе, но так как я понял не граматно
    делать то есть: 
  ...
  2.При наследовании классов если требуется добавлять методы и свойства дочернему классу на нулевой уровень, то т.к. это наследование
    требуеться использовать метод super() для расширения дочернего constructor'a родительскими методами и свойствами. 
    (Есть кстате моменты когда некоторые методы не передаются, встречал методы с _ подчёркиванием при наследовании их не было 
    в дочернем элементе надо узнать как это работает)

  Из 1 и 2го пункта встаёт вопрос: Если можно создавать методы и свойства в не конструктора и
  эффект тот же, то смысл использовать конструктор. К тому же без конструктора не требуеться указывать super()
  для передачи свойств и методов.
  
  Для чего же нужен конструктор? Без конструктора мы должны инициализировать экземпляр далее если хотим внести изменения мы просто
  обтатимся и изменим. Изменение не будет распространяться на дочерний класс, хоть в конструкторе хоть без.

constructor просто даёт возможность присваивать значения сразу же при инициализации и не более того,
но опять же при наследовании свойств в которые будут заноситься данные при инициализации экземпляра, дочерний класс
не будет знать об этих изменениях, он просто имеет функционал, а не даные как я понимаю.

И так мы выяснили что Конструктор требуется просто для за заполнения данных при инициализации. 
Вместо 2х строк: инициализации и обращении, мы запишем обну. super() нужен для того, что бы через дочерний конструктор прокинуть в родительский конструктор
какие-то значения. которые присвоим родительскм методам и свойствам. Вот и весь секрет, живи 100 лет.

	3.Методы указаные за пределами constructor находяться во вкладке __proto__ 1го уровня для себя,
    и 2го уровня для дочернего класса не загрязняя пространство. Как добавить методы и свойства в тоже пространство но за пределами
    класса, есть 2 способа: с помощью класса и с помощью экземпляра.
    через класс: nameClass.prototype.nameMethod = function(){}
    через экземпляр: nameInstance.constructor.ptototupe.nameMethod = function(){}

  4.Методы и свойства в классе с префиксом static улетают в __proto__ на 1й уровень в сам constructor. При наследовании искать их в 
    родительском конструкторе.
    Такого же эффекта можно добиться добавляя свойства и методы вне класса через имя класса без использования prototype
    NameClass.nameMethod = function(){}
    эквивалентроно создание в классе
    static nameMethod = function(){} или static nameMethod(){}
    добавление через экзмпляр таким способом просто запирает метод или свойство в этом классе и оно не наследуеться и
    доступно только для этого экземпляра
     Ещё раз 
    nameInstance.nameMethod = function(){} - доступно только для этого экземпляра и не наследуеться
    nameInstance.constructor.ptototupe.nameMethod = function(){} - общее и наследуеться 

    

  5.В классе при использовании событий теряеться контекст (this теряется), становиться undefined, 
    поэтому требуется через bind передать this этому методу. bind не вызывает функцию, а как бы засовывает её на 2й
    вызов это всё равно что наша функция оказалась в функции bind с переданым сонтекстом и чтоб её вызвать нужно дописать ()
    Все события просто дописывают () вызывая тем самым функцию поэтому можно контекст передать в событии

  6.Если требуеться что бы наследовались свойства и методы то есть 2 варианта или закидываем в prototype
    хоть через класс или экземпляры или вмешиваемся в изменение класса внутри его.
