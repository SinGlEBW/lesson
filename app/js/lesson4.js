...
/*ОПИСАНИЕ.arguments содержит все передаваемые аргументы в функцию, даже если их было переданно лишку
...
//контекст означает к кому будет принадлежать this. эеземпляру объекта.
let a = new increase('middle', 2000); 
let b = new increase('junga', 200); 
//можно не только создавать экземпляр, можно изменять часть объектов 
increase.call(em,'junga', 200); 
 /*
 bind call apply в этих функциях контекст передавать 
 нужно только тогда когда используется this в остальных случаях ставить null
*/
...
//2й способ. Принцип bind
/*
  Идея возвращать функцию заключается в том если нужно передать данные при вызове 1й функции
  и замыканием передать во вторую функцию, при этом мы вернём 2ю функцию какой-нибудь переменной
  через которую если нужно будем вызывать эту 2ю функцию. 
*/
function a4(width){

  return function b4(length){

    return function c4(height){
