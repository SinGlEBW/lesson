...
let server = http.createServer().on('request', (req,res) => {//кстате событие vscode не показывает
    ...
    res.end('okay');//без завершающего запроса сервер не запуститься
...
/*
    Отдельные запросы строятся через request или get
*/
...
/*
    rawHeaders - хранит массив заголовков которое передаются браузеру, помимо наших ещё и автоматически подкидывает.
    Записывается это в Network во вкладке Request Headers 
...
/*
   ...
   *  //ивенты относятся к элементам когда работаем через request. Некоторые ивенты могут повторяться
   *
   ...
   * Смысл в том из-за того что есть наследование разных классов отсюда и набирается список Ивентов.
...
let options = {//некоторые параметры для запроса. Работает только в запросе с request или get. с createServer такой подход не работает
    ...
    //auth: 'user:password' - Если требуется
  };
/*
 ...
 //req.write('rrrr');//Принимает буфер или строку, для отправки на сервер. Если часто использовать то в заголовку нужно указать Transfer-Encoding: 'chunked' 
 req.end();
*/
// закидывается почему то в body сайта при использовании createServer
 ...
 *   client: Socket { тут всё повторяется }
 *   IncomingMessage {...}//выводиться снова этот объект
 * } 
 * потом снова IncomingMessage и всё повторяется. Ебала какая-то с NodeJS многое по 2 раза срабатывает
 * 
 * параметр Responce содержит объект ServerResponce
 * ServerResponse{
 *   _events: {...}
 *   socket: Socket{
 *       _readableState: ReadableState {...}  
...
/*#########----Как выводят страницы на NodeJS----########*/
