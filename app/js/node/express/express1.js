...
/*
    ...
    Что нам это даёт. При ручном добавлении файлов к запросам, требуеться отследить все запросы, после добавлять нужные файлы к каждому запросу.
...
app.get('/news/:id', (req, res) => {
    ...
    if(req.params.id === 'main.js'){
        ...
        res.render('news', {obj})//безымянный объект полюбому нужен
    }
...
/*  ####---ЗАМЕТКИ---####
    Правтика показала что в запросах не работает полноценно регурярное выражение, а в
    частности не выходит собрать один запрос такиим образом что бы проверялось
    URL группами из возможных страниц, тоесть не работает логика "ИЛИ () | ()" в запросе
    Динамический объект params просто отказываеться заполняться при наличии "ИЛИ" 
    ...
    По поводу favicon. Дулаю вывод что браузер в своём непредсказуемом режиме делает запрос на иконку.
    ...
    По всей видимости не важно что в console адресс ..app\\css/style.min.css наверно методы чтения 
    сами выравнивают адресс.
    ...
    встречаеться одна и та же обработка запроса, то отрабатывать будет 1й запрос.
    Сначало отрабатывает запрос на страницу html, а потом другие запросы которые требует эта страница.
    Есть метод "use" который работает паралельно запросу.
    ...
    Получаеться что все пути к файлам указываються в разных файлах относительно файла в котором пишем код.
    ...
    для этого нужно подавать файлы тогда когда они запрашиваються, и следить
    ...
    Метод "use" вызываеться перед самим запросом get. Этот метод может делать абра кадабру, но следующий запрос не будет отработан.
    Что бы следующий запрос мог отработать у метода "use" есть 3й параметр в callback фнкции
    называеться next() - эта функция просто напросто вызывает следующий запрос. Поэтому он и считаеться промежуточным.
    Другие запросы тоже могут быть промежуточными и что бы вызвать цепочку других запросов 
    вызываеться next() в запросе.
    В чём смысл если всё что мне надо я могу выполнить в самом запросе get. или же воспользоваться однем use
    ...
    <%- Выводит значение в шаблон HTML неэкранированного типа. Тоесть если передать html код, то нужно использовать этот тег
    <%# Тег комментария
    <%% Выводит литерал '<%'
    %> конец тега
