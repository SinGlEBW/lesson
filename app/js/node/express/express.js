...
/*
    Middleware функция use срабатывает на все запросы серваку, в то время как
    Middleware функции в запросах можно контролировать через next 
...
/*
    О Middleware функциях 
    Это те функции которые выполняются асинхронно, то есть друг за другом.
    app.get("patch", (req, res, next) => {
        console.dir(1);
        next();//вызов следующей
    ...
    (req, res, next) => {
        console.dir(3.Последняя);
    });
    Чем хорош express, это тем что можно передавать массив 2м параметром, который будет выполняться 
    друг за другом без лишних next() вызовов
     
    let arr = [действие1, действие2, действие3];
    app.get("patch", arr, (req, res, next) => {
        console.dir(arr);
        next();//вызов следующей
       
    });
   На самом же деле каждый элемент массива помещается в такой же callback (Middleware функцию)
   и помещаются они друг за другом. Каждая функция запускает next
...
*/
/*
// с динамический URL и регулярный подход нужно отделять группами иначе не сработает
